var MAIN = (function(){
	...
	function attemptCreateReport(ce){
		if(window.COFFEE_CONFIG.API_GW_REPORT_URL_STR === null){
		...
		$.ajax({
			url: window.COFFEE_CONFIG.API_GW_REPORT_URL_STR + "/" + "create_report",
			method: "POST",
			data: {},
			headers: {
	...
	function handleReportCreationSuccess(response){
		...
		if(response.msg){
			showProblem("Se ha generado el reporte, pronto recibira un correo con el resguardo de la operacion:\n\n" + response.msg);
		}
